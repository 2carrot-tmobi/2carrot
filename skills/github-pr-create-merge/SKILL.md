...
7. PR 생성 전 source/target compare 내역이 있는지 확인한다.
   ...
   - source/target 최근 커밋 목록을 따로 나열해 수동 비교하지 않는다
...
8. PR 제목을 만든다.
   - 사용자가 body/title을 주지 않았다면 compare 범위의 첫 번째 non-merge commit 제목을 우선 사용한다
   - compare 전용 데이터가 없으면 source head의 최신 non-merge commit 제목을 사용한다
   - 형식: `<대표 제목> -> <target>`
   - 적합한 제목이 없으면 `chore: <source> -> <target>`
9. PR 본문을 만든다.
   - 사용자가 body를 주면 그대로 사용
   - 없으면 compare 범위 전체를 기준으로 요약하고, compare 전용 데이터가 없으면 source 쪽 신규 non-merge commit들을 기준으로 요약한다
...
- source 브랜치와 target 브랜치의 최근 커밋 목록을 각각 조회해 사람이 수동으로 대조하는 방식은 사용하지 않는다.
- merge commit은 compare 범위의 제목/본문 계산에서 제외한다.
- 모드 B는 자동 머지하지 않는다.
