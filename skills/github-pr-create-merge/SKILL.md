...
- Wiki page
  ...
  - 주어지지 않으면 `TMAPDRIVER` space에서 대상 날짜 기준으로 검색한다
...
15. 모드 C에서는 위키를 업데이트한다.
   ...
   - 위키 페이지가 주어지지 않으면 `TMAPDRIVER` space에서 위키를 검색한다
   - 검색 우선순위:
     - 요청한 배포일 `{yyyy.MM.dd}` row가 실제로 들어 있는 페이지
     - 제목이 `{년도}년 배포 내역 ({시작월}~{종료월})` 패턴인 페이지
   - 위키 페이지가 정해지면 해당 날짜 행을 찾고 배포 애플리케이션 항목을 반영한다
...
- 위키를 자동 선택할 때는 `TMAPDRIVER` space에서 `{년도}년 배포 내역 ({시작월}~{종료월})` 패턴 페이지를 찾는다.
- 요청한 배포일 `{yyyy.MM.dd}` row가 이미 있는 페이지를 패턴 일치만 하는 페이지보다 우선 선택한다.
